          ...
          system_update() {
              update_system() {
                  if command -v "nixos-rebuild" &>/dev/null; then
                    ...
                    nixos_tmp=$(mktemp -d)
                    ${pkgs.git}/bin/git clone --depth 1 https://github.com/furioncycle/nixos-config "$nixos_tmp" > /dev/null 2>&1
                    cd $nixos_tmp
              ...
              update_home() {
                  if command -v "home-manager" &>/dev/null; then
                    ...
                    ${pkgs.git}/bin/git clone https://github.com/furioncycle/hagor "$nixhome_tmp" > /dev/null 2>&1
                    cd $nixhome_tmp
